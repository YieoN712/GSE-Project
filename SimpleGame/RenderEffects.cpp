...
#version 120
        ...
        void main()
        {
            ...
            float roughness = .85;
            if (material == 1)
            { // grass: fine blades and broad, nonuniform patches
            ...
            { // standing-seam roof
                color *= .87 + .10 * grain;
                color *= mix(.62, 1.0, step(.055, fract(uv.x * 2.3)));
                roughness = .35;
            }
            else if (material == 6)
            { // blue glass, restrained sky reflection
                color = mix(color, vec3(.66, .82, .85), .22 + .16 * sin(uv.y * 1.8));
                roughness = .12;
            }
            else if (material == 7)
            {
            ...
            float shade = 0.0;
            if (shadowEnabled > .5 && p.x > .002 && p.x < .998 && p.y > .002 && p.y < .998 &&
                p.z > 0.0 && p.z < 1.0)
            {
            ...
            float spec = pow(max(dot(n, halfway), 0.0), mix(70.0, 8.0, roughness)) *
                         (1.0 - roughness) * .22 * (1.0 - shade);
...
#version 120
        ...
        void main()
        {
            ...
            float edge = max(max(luma(a), luma(b)), max(luma(d), luma(e))) -
                         min(min(luma(a), luma(b)), min(luma(d), luma(e)));
            c = mix(c, (a + b + d + e + c * 4.0) / 8.0, smoothstep(.08, .28, edge) * .55);
            vec3 bloom = vec3(0);
            for (int x = -2; x <= 2; ++x)
                for (int y = -2; y <= 2; ++y)
                    bloom +=
                        max(texture2D(scene, uv + vec2(float(x), float(y)) * texel * 3.0).rgb - .82,
                            0.0);
            c += bloom / 25.0 * .22;
            c = (c - .5) * 1.035 + .5;
...
void RenderEffects::BeginScene(int width, int height)
{
    ...
    if (enabled)
        glBindFramebuffer(GL_FRAMEBUFFER, sceneActive ? sceneFbo : 0);
    ...
    if (enabled)
    {
        ...
        glUniformMatrix4fv(
            glGetUniformLocation(sceneProgram, "lightMatrix"), 1, GL_FALSE, lightMatrix);
